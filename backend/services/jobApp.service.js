...
const createJobApp = async (jobId, data) => {
  try {
    ...
    if (
      job.isCoverLetterNeeded &&
      (!data["coverLetter"] || data["coverLetter"].length === 0)
    ) {
    ...
    await User.findByIdAndUpdate(userId, {
      $push: { jobApps: newApplication._id },
    });
...
const deleteJobApp = async (jobAppId, data) => {
  try {
    ...
    if (data.userId !== jobApp.userId.toString()) {
      return {
        status: 403,
        message: "Forbidden: Job App does not correspond to user!",
      };
    }

    await JobApp.deleteOne({ _id: jobAppId });

    await User.updateOne({ _id: user._id }, { $pull: { jobApps: jobAppId } });

    await CV.updateOne({ _id: jobApp.cvId }, { $pull: { jobApps: jobAppId } });
...
const getJobAppDetails = async (jobAppId, userId) => {
  try {
    ...
    if (!(await isHirer(jobApp.jobId, userId))) {
      ...
      return {
        status: 403,
        message: "Forbidden: Only hirer can view Job App!",
      };
    }
...
const getMyJobApps = async (userId) => {
  try {
    ...
    const applications = await JobApp.find({
      _id: { $in: user.jobApps.map((id) => new mongoose.Types.ObjectId(id)) },
    })
      .populate({
        path: "jobId",
        populate: {
          path: "postedBy",
          model: "User" // Only fetch the necessary fields
        },
      }) // Assuming 'jobId' is the field name in JobApp model that refers to Job
      .populate("cvId");
